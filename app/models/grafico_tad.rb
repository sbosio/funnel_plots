class GraficoTad < ActiveRecord::Base
  include UserStamp
  ...
  def obtener_grafico
    ...
    # Trabajamos con tasas ajustadas directamente, así que debemos calcular los pesos ('w_sub_j') para
    # cada categoría de la covariable con respecto al total de población
    serie_w = {}
    poblacion.covariable.categorias_de_la_covariable.each do |categoria|
      serie_w.merge!(
          categoria => poblacion.datos.where(
              categoria_de_la_covariable_id: categoria.id
            ).sum(:valor) / poblacion.datos.sum(:valor))
    end
    ...
    # Serie de tasas específicas por categoría para cada unidad de análisis
    serie_Y_sub_j = {}
    poblacion.conjunto_de_unidades_de_analisis.unidades_de_analisis.each do |unidad|
      poblacion.covariable.categorias_de_la_covariable.each do |categoria|
        serie_Y_sub_j.merge!(
            [unidad, categoria] => (
                eventos_observados.datos.where(
                    unidad_de_analisis_id: unidad.id,
                    categoria_de_la_covariable_id: categoria.id
                  ).sum(:valor) /
                poblacion.datos.where(
                    unidad_de_analisis_id: unidad.id,
                    categoria_de_la_covariable_id: categoria.id
                  ).sum(:valor)
              )
    ...
    serie_N.each do |unidad, valor|
      ...
      serie_Y.merge!(
          unidad => (
              poblacion.covariable.categorias_de_la_covariable.map do |categoria|
                  serie_Y_sub_j[[unidad, categoria]] * serie_w[categoria]
                end
            ).sum * mult
        )
    end
